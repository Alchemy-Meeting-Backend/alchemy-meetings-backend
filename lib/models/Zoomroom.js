...
module.exports = class Zoomroom {
  ...
  static async getAll(cohort_id) {
    const { rows } = await pool.query(
      `SELECT 
    zoom_rooms.id, zoom_rooms.room_name, zoom_rooms.meeting_id,
    COALESCE (
...
`,
  ...
  //   static async getByRoomType(type) {
  //     const { rows } = await pool.query('SELECT * from zoom_rooms WHERE type= $1',
  //     [type])
  //   }
};

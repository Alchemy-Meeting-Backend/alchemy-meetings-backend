...
module.exports = Router()
  ...
  .put('/:id', authenticate, authorizeAdmin, async (req, res, next) => {
    try {
      const userInfo = await GithubUser.update(req.params.id, req.body);
      res.json(userInfo);
    } catch (error) {
      next(error);
    }
  ...
  .delete('/:id', authenticate, authorizeAdmin, async (req, res, next) => {
    try {
      const userInfo = await GithubUser.delete(req.params.id);
      res.json(userInfo);
    } catch (error) {
      next(error);
    }
  ...
  .get('/cohorts/:cohort_id', authenticate, authorizeAdmin, async (req, res, next) => {
    try {
      const specificCohort = await GithubUser.getUsersByCohortId(
        req.params.cohort_id
      );
      res.json(specificCohort);
    } catch (error) {
      next(error);
    }
  });
